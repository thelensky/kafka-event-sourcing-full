package com.example.billingservice.billing.listener;

import com.example.kafkaes.avro.OrderCreatedAvro;
import com.example.billingservice.shared.model.ProcessedEvent;
import com.example.billingservice.shared.repo.ProcessedEventRepository;
...
public class BillingListener {
    ...
    public BillingListener(ProcessedEventRepository repo) {
        this.repo = repo;
    }

    @KafkaListener(topics = "orders", groupId = "billing-service_group")
    @Transactional
    public void onOrder(OrderCreatedAvro event) {
        String id = event.getOrderId().toString();
        if (repo.existsByEventId(id)) {
            System.out.println("[Billing] already processed " + id);
            return;
        }
